...
class FrontController extends AppBaseController
{
     ...
     * @return Application|Factory|View
     */
    public function servicesCategory($category_id)
    {
        $data = [];
        $serviceCategory = ServiceCategory::find($category_id);
        $setting = Setting::pluck('value', 'key')->toArray();
        $services = Service::with('media')->whereStatus(Service::ACTIVE)->where('category_id', $category_id)->latest()->get();
        $data['doctorsCount'] = Doctor::with('user')->get()->where('user.status', true)->count();
        $data['patientsCount'] = Patient::get()->count();
        $data['servicesCount'] = Service::whereStatus(true)->get()->count();
        $data['specializationsCount'] = Specialization::get()->count();

        return view('fronts.medical_category_services', compact('serviceCategory','setting','services','data'));
    }
    ...
    {
        $faqs = Faq::latest()->get();
        $appointmentDoctors = Doctor::with('user')->get()->where('user.status',
